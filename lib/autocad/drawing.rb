...
module Autocad
  class Drawing
    ...
    # @param name [String] The name of the layer to create
    # @param color [Integer, Symbol, String] The color for the layer (can be ACAD::COLOR constant, symbol, or integer)
    # @return [Autocad::Layer] The created or existing layer
    def create_layer(name, color = nil)
      # Check if layer already exists
      existing = layers.find { |l| l.name.casecmp?(name) }
      return existing if existing

      # Create new layer
      layer = ole_obj.Layers.Add(name)
      layer_obj = app.wrap(layer)
      
      # Set color if provided
      if color
        layer_obj.color = Autocad.color_to_index(color)
      end
    ...
    # @return [Autocad::Layer] The active layer
    def active_layer
      app.wrap(ole_obj.ActiveLayer)
    end

    # Set the active layer in the drawing
    # @param layer [String, Autocad::Layer] The layer name or layer object to set as active
    def active_layer=(layer)
      if layer.is_a?(String)
        # Find the layer by name
        layer_obj = layers.find { |l| l.name == layer }
        raise Autocad::Error, "Layer '#{layer}' not found" unless layer_obj
        ole_obj.ActiveLayer = layer_obj.ole_obj
      else
        # Assume it's a Layer object
        ole_obj.ActiveLayer = layer.ole_obj
      end
    end
    ...
    # @yield [Autocad::Layer] Each layer in the drawing
    # @return [Enumerator] An enumerator of layers if no block given
    def layers
      return enum_for(:layers) unless block_given?

      ole_obj.Layers.each do |layer|
        yield app.wrap(layer)
      end
    end

    # Get the model space of the drawing
    # @return [Autocad::ModelSpace] The model space
    def model_space
      app.wrap(ole_obj.ModelSpace)
    end
    alias_method :model, :model_space
    ...
    # @return [Autocad::PaperSpace] The paper space
    def paper_space
      app.wrap(ole_obj.PaperSpace)
    end
    alias_method :paper, :paper_space
    ...
    # @return [void]
    def to_model_space
      ole_obj.ActiveSpace = 0 # acModelSpace
    end

    # Switch to paper space
    # @return [void]
    def to_paper_space
      ole_obj.ActiveSpace = 1 # acPaperSpace
    end
    ...
    # Get the active space (model or paper)
    # @return [Autocad::ModelSpace, Autocad::PaperSpace] The active space
    def active_space
      model_space? ? model_space : paper_space
    end

    # Create a selection set in the drawing
    # @param name [String] The name for the selection set
    # @yield [Autocad::SelectionSet] The created selection set
    # @return [Autocad::SelectionSetAdapter] The created selection set
    def create_selection_set(name)
      ss = Autocad::SelectionSet.new(name)
      adapter = Autocad::SelectionSetAdapter.new(self, ss)
      yield ss if block_given?
      adapter
    end

    # Get all selection sets in the drawing
    # @yield [Autocad::SelectionSetAdapter] Each selection set
    # @return [Enumerator] An enumerator of selection sets if no block given
    def selection_sets
      return enum_for(:selection_sets) unless block_given?

      ole_obj.SelectionSets.each do |ss|
        yield Autocad::SelectionSetAdapter.from_ole_obj(self, ss)
      end
    end
    ...
    def name
      File.basename(ole_obj.FullName)
    end

    # Get the path of the drawing
    # @return [Pathname] The path of the drawing
    def path
      Pathname.new(ole_obj.FullName)
    end

    # Get the basename of the drawing
    # @return [Pathname] The basename of the drawing
    def basename
      Pathname.new(name)
    end

    # Get the directory of the drawing
    # @return [Pathname] The directory of the drawing
    def dirname
      path.dirname
    end

    # Save the drawing
    # @param name [String] Optional new name for the drawing
    # @param dir [String, Pathname] Optional directory to save to
    # @return [void]
    def save(name: nil, dir: nil)
      if name || dir
        save_as(name: name, dir: dir)
      else
        ole_obj.Save
      end
    end
    ...
    # @param name [String] New name for the drawing
    # @param dir [String, Pathname] Directory to save to
    # @return [void]
    def save_as(name: nil, dir: nil)
      new_path = if dir
                   dir_path = Pathname.new(dir)
                   file_name = name || self.name
                   dir_path + file_name
                 elsif name
                   dirname + name
                 else
    ...
    # Get the OLE object for the drawing
    # @return [WIN32OLE] The OLE object
    def ole_obj
      @ole_obj
    end

    # Get all blocks in the drawing
    # @yield [Autocad::Block] Each block in the drawing
    # @return [Enumerator] An enumerator of blocks if no block given
    def blocks
      return enum_for(:blocks) unless block_given?

      ole_obj.Blocks.each do |block|
        yield app.wrap(block)
      end
    end

    # Get all linetypes in the drawing
    # @yield [Autocad::Linetype] Each linetype in the drawing
    # @return [Enumerator] An enumerator of linetypes if no block given
    def linetypes
      return enum_for(:linetypes) unless block_given?

      ole_obj.Linetypes.each do |lt|
        yield app.wrap(lt)
      end
    end

    # Get all layouts in the drawing
    # @yield [Autocad::Layout] Each layout in the drawing
    # @return [Enumerator] An enumerator of layouts if no block given
    def layouts
      return enum_for(:layouts) unless block_given?

      ole_obj.Layouts.each do |layout|
        yield app.wrap(layout)
      end
    end

    # Get all plot configurations in the drawing
    # @yield [Autocad::PlotConfiguration] Each plot configuration
    # @return [Enumerator] An enumerator of plot configurations if no block given
    def plot_configurations
      return enum_for(:plot_configurations) unless block_given?

      ole_obj.PlotConfigurations.each do |config|
        yield app.wrap(config)
      end
    end

    # Create a PDF plot configuration
    # @return [Autocad::PlotConfiguration] The created plot configuration
    def pdf_plot_config
      # Implementation depends on specific requirements
      # This is a placeholder based on the test
      config = ole_obj.PlotConfigurations.Add("faa_ansid_bw")
      app.wrap(config)
    end

    # Get the first non-Model layout
    # @return [Autocad::Layout] The first paper space layout
    def paper_space_layout
      layouts.find { |layout| layout.name != "Model" }
    end

    # Get system variables
    # @param *vars [Array<String>] Variable names to get
    # @return [Array] Values of the requested variables
    def get_variables(*vars)
      vars = vars.first if vars.first.is_a?(Array)
      vars.map { |var| ole_obj.GetVariable(var) }
    end

    # Set system variables
    # @param names [Array<String>] Variable names to set
    ...
    # Get the event handler for the drawing
    # @return [Autocad::EventHandler] The event handler
    def event_handler
      @event_handler ||= Autocad::EventHandler.new
    end

    # Register an event handler
    # @param event [String] The event name
    # @yield The handler block
    # @return [void]
    def register_handler(event, &block)
      event_handler.add_handler(event, &block)
    end

    # Regenerate the drawing
    # @param mode [Symbol] Regeneration mode (:all or :active)
    # @return [void]
    def regen(mode = :all)
      case mode
      when :all
        ole_obj.Regen(0) # acAllViewports
      when :active
        ole_obj.Regen(1) # acActiveViewport
      end
    end

    # Save the drawing as PDF
    # @param name [String] Optional name for the PDF
    # @param dir [String, Pathname] Directory to save to
    # @param model [Boolean] Whether to use model space
    # @return [void]
    def save_as_pdf(name: nil, dir: nil, model: false)
      # Implementation depends on specific requirements
      # This is a placeholder based on the test
      # Actual implementation would use the PDF plot configuration
    end

    # Copy the drawing
    # @param name [String] Optional new name
    # @param dir [String, Pathname] Directory to copy to
    # @return [void]
